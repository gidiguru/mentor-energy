...
const Layout = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="flex h-screen"> ${`<aside class="w-64 bg-surface-100-800-token border-r border-surface-500/30 p-4 hidden sm:block"><div class="flex flex-col gap-2">${each(navItems, (item) => {
    return `<div class="w-full"><button class="${"btn w-full " + escape(
      $currentPath === item.path ? "variant-filled-primary" : "variant-ghost",
      true
    ) + " justify-start svelte-tlu51z"}"><span class="flex items-center gap-3"><span class="text-xl">${escape(item.icon)}</span> <span>${escape(item.label)}</span> ${item.hasSubItems ? `<span class="ml-auto">${escape(expandedItem === item.path ? "▼" : "▶")}</span>` : ``} </span></button> ${item.hasSubItems && expandedItem === item.path ? `<div class="bg-surface-200-700-token rounded-b-lg">${each(item.subItems, (subItem) => {
      return `<button class="${"btn w-full pl-8 " + escape(
        $currentPath === subItem.path ? "variant-filled-primary" : "variant-ghost",
        true
      ) + " svelte-tlu51z"}"><span class="flex items-center gap-3"><span class="text-xl">${escape(subItem.icon)}</span> <span>${escape(subItem.label)}</span></span> </button>`;
    })} </div>` : ``} </div>`;
  })} <button class="btn variant-ghost-error mt-auto svelte-tlu51z" data-svelte-h="svelte-v7p5xh"><span class="flex items-center gap-3"><span class="text-xl">🚪</span> <span>Sign Out</span></span></button></div></aside>`}  <div class="flex-1 flex flex-col"> ${``}  <main class="flex-1 p-4 overflow-auto">${slots.default ? slots.default({}) : ``}</main></div> </div>`;
});
export {
  Layout as default
};
